...
void ICACHE_FLASH_ATTR handleEVSETimer()
{
  ...
  if (!timerFile)
  {
    ...
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ ERR ] Timer File does not exist");
    return;
  }
  ...
  if ((!config.getEvseAlwaysActive(0) && timerToDeactivate && timerActive == true && timerGlobalActive) || // Timer run out or
      ...
      (!config.getEvseAlwaysActive(0) && timerGlobalActive == false && timerActive == true))
  { // Timer globally deactivated but timerActive == true
...
void ICACHE_FLASH_ATTR handleRse()
{
  ...
  { // RSE goes activated
    ...
    if (currentToSet > 0 && currentToSet < 6)
      currentToSet = 6;
    if (config.getEvseRseValue(0) == 0)
      currentToSet = 0;
    sliderStatus = false;
    if (config.getSystemDebug())
      slog.log(ntp.iso8601DateTime() + "[ SYSTEM ] RSE Interrupted! Setting current to ");
    if (config.getSystemDebug())
      slog.logln(currentToSet);
  }
  else
  { // RSE goes deactivated
    toSetEVSEcurrent = true;
    currentToSet = currentBeforeRse;
    if (!config.getEvseRemote(0))
      sliderStatus = true;
    if (config.getSystemDebug())
      slog.log(ntp.iso8601DateTime() + "[ SYSTEM ] RSE Released! Setting current back to ");
    if (config.getSystemDebug())
      slog.logln(currentToSet);
  }
}
...
{ // interrupt routine for metering
...
{
  if (config.getSystemDebug())
    slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Factory Reset...");
  SPIFFS.remove("/config.json");
  initLogFile();
  if (resetUserData())
  {
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] ...successfully done - going to reboot");
  }
  toReboot = true;
  delay(100);
  return true;
...
bool ICACHE_FLASH_ATTR reconnectWiFi()
{
  ...
  if (config.getSystemDebug())
    slog.log(ntp.iso8601DateTime() + "[ INFO ] Trying to reconnect WiFi without given BSSID: ");
  if (config.getSystemDebug())
    slog.logln(config.getWifiSsid());
  return true;
}
...
slog.logln(scan.valid); */
    ...
    { // In AA/Remote just turnOnOled and leave
      if (millisOnTimeOled < millis())
      { // Init lastRFIDUID if OLED is on (second read is needed for RFID Action)
        turnOnOled();
        lastRFIDUID = "";
      }
      else
    ...
    if (!config.getEvseAlwaysActive(0) && !timerActive && // If oLED is turned off, just turn it on and leave
        millisOnTimeOled < millis())
    { // To activate EVSE, a second RFID read is necessary
      turnOnOled();
      return;
    }
#endif
...
void ICACHE_FLASH_ATTR logLatest(String uid, String username)
{
  ...
  if (logFile)
  {
    ...
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Impossible to read log file");
    }
    else
    {
      logFile.close();
  ...
  {
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Cannot create Logfile");
  }
  // SPIFFS.end();
  delay(100);
  fsWorking = false;
...
void ICACHE_FLASH_ATTR readLogAtStartup()
{
  ...
  else
  {
    ...
    if (config.useMMeter)
      startTotal = list[(list.size() - 1)]["reading"];
    vehicleCharging = true;
    fsWorking = false;
  }
  return;
...
void ICACHE_FLASH_ATTR updateLog(bool incomplete)
{
  ...
  if (config.getSystemDebug())
    slog.logln(ntp.iso8601DateTime() + "[ LOG ] Update Logfile...");
  fsWorking = true;
  delay(30);
  File logFile = SPIFFS.open("/latestlog.json", "r");
  size_t size = logFile.size();
  ...
  if (error)
  {
    logFile.close();
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Impossible to update log file");
    slog.log(ntp.iso8601DateTime() + "[ SYSTEM ] Impossible to parse Log file: ");
    slog.logln(error.c_str());
  }
  else
  {
    ...
    if (!incomplete)
    {
      ...
      if (config.getWifiWmode() && getChargingTime() > 360000000)
      ...
      if (config.useMMeter)
        jsonDoc2["reading"] = startTotal;
      if (config.useMMeter)
        jsonDoc2["rEnd"] = endTotal;
    }
    else
    {
      jsonDoc2["duration"] = String("e");
    ...
    if (logFile)
    {
      ...
      for (int i = 0; i < 2; i++)
      {
        ...
        {
          if (config.getSystemDebug())
            slog.logln(ntp.iso8601DateTime() + "[ LOG ] LogFile verified!");
          break;
        }
        else
        {
...
bool ICACHE_FLASH_ATTR initLogFile()
{
  ...
  fsWorking = true;
  if (config.getSystemDebug())
    slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Going to delete Log File...");
  File logFile = SPIFFS.open("/latestlog.json", "w");
  if (logFile)
  {
    StaticJsonDocument<35> jsonDoc;
    ...
    logFile.close();
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] ... Success!");
    ret = true;
  }
  else
  {
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] ... Failure!");
    ret = false;
  }
  delay(100);
  fsWorking = false;
  ...
  if (!config.getEvseAlwaysActive(0) && !timerActive)
  {
    ...
    { // EVSE not Ready
          ...
          evseVehicleState == 4)
      {
        ...
        if (config.getEvseLedConfig(0) == 3)
          changeLedTimes(300, 2000);
      }
      else
      {
        evseStatus = 1; // EVSE deactivated
      }
      if (vehicleCharging == true && manualStop == false)
      { // vehicle interrupted charging
        ...
        if (config.getSystemDebug())
          slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Vehicle interrupted charging");
        updateLog(false);
      }
      evseActive = false;
      return true;
  ...
  else
  {
    ...
    { // PWM is being generated
      ...
      { // EV is charging
        ...
        { // EV starts charging
          ...
          lastUsername = "vehicle";
          if (config.getSystemDebug())
            slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Vehicle started charging");
        }
        evseStatus = 3; // charging
        evseActive = true;
        if (config.getEvseLedConfig(0) == 3)
    ...
    { // EVSE not Ready
          ...
          evseVehicleState == 4)
      {
        ...
        if (config.getEvseLedConfig(0) == 3)
          changeLedTimes(300, 2000);
        if (vehicleCharging && evseAmpsConfig == 0)
        { // Current Set to 0 - deactivate
          ...
          lastUsername = "API";
          if (config.getSystemDebug())
            slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] API interrupted charging");
        }
      }
      else
      {
...
bool ICACHE_FLASH_ATTR activateEVSE()
{
  ...
  { // Normal Mode
    ...
    {                                          // no modbus error occured
      ...
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ ModBus ] EVSE successfully activated");
    }
  }
  else
  {
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ ModBus ] EVSE already active");
  }
...
bool ICACHE_FLASH_ATTR deactivateEVSE(bool logUpdate)
{
  ...
  { // Normal Mode
    ...
    // register successfully written
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ ModBus ] EVSE successfully deactivated");
    evseActive = false;
    stopChargingTimestamp = ntp.getUtcTimeNow();
  }
  manualStop = true;
  ...
  if (config.getSystemDebug())
    slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] EVSE successfully deactivated");
  return true;
}

bool ICACHE_FLASH_ATTR setEVSEcurrent()
{ // telegram 1: write EVSE current
  // New ModBus Master Library

  if (highResolution)
  {
    if (currentToSet < 64)
    { // Current given in low resolution
  ...
  {
    if (currentToSet > 64)
    { // Current given in high resolution
      currentToSet = currentToSet / 100;
    }
  }
  ...
  if (config.getEvseRemote(0))
  {
    ...
    if (currentToSet == evseAmpsConfig)
      return true; // no oLED action
    turnOnOled(5);
  }
  else
  {
    ...
    toSetEVSEcurrent = false;
    if (currentToSet == evseAmpsConfig)
      return true; // oLED action but register is already set
  }
...
bool ICACHE_FLASH_ATTR setSDMID()
{
  ...
  if (config.useMMeter)
  { // short delay to prevent collisions
    if (millisUpdateMMeter - millis() < 50)
    {
      delay(50);
    }
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    else
    {
      // register successfully written
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ ModBus ] Register 0x0014 successfully set to 2");
...
bool ICACHE_FLASH_ATTR setEVSERegister(uint16_t reg, uint16_t val)
{
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    else
    {
      // register successfully written
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ ModBus ] Register " + (String)reg + " successfully set to " + (String)val);
      millisUpdateEvse = millis() + 100;
      return true;
    }
  }
...
void ICACHE_FLASH_ATTR onWsEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len)
{
  if (type == WS_EVT_ERROR)
  {
    ...
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + errText);
  ...
  else if (type == WS_EVT_DATA)
  {
    ...
    if (info->final && info->index == 0 && info->len == len)
    {
      ...
      {
        if (config.getSystemDebug())
          slog.logln(ntp.iso8601DateTime() + "[ WARN ] Couldn't parse WebSocket message");
        msg = "";
        return;
      }
    ...
    else
    {
      ...
      if (config.getSystemDebug())
        slog.log(ntp.iso8601DateTime() + "[ Websocket ] more than one Frame!");
...
void ICACHE_FLASH_ATTR processWsEvent(JsonDocument &root, AsyncWebSocketClient *client)
{
  ...
  {
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Try to update config.json...");
    String configString;
    ...
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Success - going to reboot now");
      if (vehicleCharging)
      {
        deactivateEVSE(true);
        delay(100);
    ...
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Could not save config.json");
    }
  }
  ...
  else if (strcmp(command, "userfile") == 0)
  {
    ...
    if (userFile)
    {
      userFile.print(msg);
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Userfile written!");
    }
    userFile.close();
  ...
  else if (strcmp(command, "latestlog") == 0)
  {
    ...
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ ERROR ] File system is already working...");
    }
  }
  ...
  else if (strcmp(command, "setcurrent") == 0)
  {
    ...
    if (config.getSystemDebug())
      slog.log(ntp.iso8601DateTime() + "[ WebSocket ] Call setEVSECurrent() ");
    if (config.getSystemDebug())
      slog.logln(currentToSet);
    toSetEVSEcurrent = true;
  }
  else if (strcmp(command, "activateevse") == 0)
  {
    toActivateEVSE = true;
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ WebSocket ] Activate EVSE via WebSocket");
    lastUID = "GUI";
    lastUsername = "GUI";
  }
  else if (strcmp(command, "deactivateevse") == 0)
  {
    toDeactivateEVSE = true;
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ WebSocket ] Deactivate EVSE via WebSocket");
    lastUID = "GUI";
    lastUsername = "GUI";
  }
  ...
  else if (strcmp(command, "resetuserdata") == 0)
  {
    ...
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ WebSocket ] User Data Reset successfully done");
    }
  }
  else if (strcmp(command, "initlog") == 0)
  {
    if (config.getSystemDebug())
      slog.log(ntp.iso8601DateTime() + "[ SYSTEM ] Websocket Command \"initlog\"...");
    toDeactivateEVSE = true;
    toInitLog = true;
  }
  ...
  {
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Try to update timer.json...");
    File timerFile = SPIFFS.open("/timer.json", "w+");
    if (timerFile)
    {
      serializeJsonPretty(root, timerFile);
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ DEBUG ] Timer file written!");
    }
    timerFile.close();
    ws.textAll("{\"command\":\"result\",\"resultof\":\"userfile\",\"result\": true}");
  }
  ...
  {
    if (config.getSystemDebug())
      slog.log(ntp.iso8601DateTime() + "[ SYSTEM ] Websocket Command \"interruptcp\"...");
    interruptCp();
  }
#endif
  ...
  else if (strcmp(command, "setnumphases") == 0)
  {
    ...
    if (config.getSystemDebug())
      slog.logln("[ SYSTEM ] Websocket Command \"setnumphases\"...");
  ...
  else if (strcmp(command, "setmeterfactor") == 0)
  {
    ...
    if (config.getSystemDebug())
      slog.logln("[ SYSTEM ] Websocket Command \"setmeterfactor\"...");
...
uint16_t onGetMbTCPIreg(TRegister *reg, uint16_t val)
{
  ...
  case 30001:
    if (config.useMMeter)
      return uint16_t(currentP1 * 100.0);
    if (config.useSMeter)
      return uint16_t(_currentP1 * 100.0);
    break;
  case 30002:
    if (config.useMMeter)
      return uint16_t(currentP2 * 100.0);
    if (config.useSMeter)
      return uint16_t(_currentP2 * 100.0);
    break;
  case 30003:
    if (config.useMMeter)
      return uint16_t(currentP3 * 100.0);
    if (config.useSMeter)
      return uint16_t(_currentP3 * 100.0);
    break;
  case 30004:
    if (config.useMMeter)
      return uint16_t(voltageP1 * 100.0);
    if (config.useSMeter)
      return uint16_t(230 * 100);
    break;
  case 30005:
    if (config.useMMeter)
      return uint16_t(voltageP2 * 100.0);
    if (config.useSMeter && (config.getMeterFactor(0) == 2 || config.getMeterFactor(0) == 3 || config.getMeterPhaseCount(0) == 3))
      return uint16_t(230 * 100);
    return 0;
    break;
  case 30006:
    if (config.useMMeter)
      return uint16_t(voltageP3 * 100.0);
    if (config.useSMeter && (config.getMeterFactor(0) == 3 || config.getMeterPhaseCount(0) == 3))
      return uint16_t(230 * 100);
    return 0;
    break;
...
bool checkUart(HardwareSerial *Ser, uint8_t deviceId)
{
  ...
  {
    if (deviceId == 1)
      result = testNode.readHoldingRegisters(0x03E8, 2); // read 1 registers starting at 0x03E8 (1000)
    if (deviceId == 2)
      result = testNode.readHoldingRegisters(0x0156, 2); // read 1 registers starting at 0x0156 (342)
    testNode.clearTransmitBuffer();
    testNode.clearResponseBuffer();
    if (result != 0)
    {
...
bool ICACHE_FLASH_ATTR loadConfiguration(String configString = "")
{
  ...
  if (config.getSystemDebug())
    slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Check for old config version and renew it");
  ...
  {
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ Modbus ] Error getting additional EVSE data!");
    delay(500);
    getAdditionalEVSEData();
    if (evseErrorCount > 2)
    {
  ...
  if (config.getEvseAlwaysActive(0))
  {
    ...
    if (addEvseData.evseReg2005 != reg2005DefaultValues)
    {
      ...
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ INFO ] EVSE register 2005 set to 0 -> Always Active Mode");
    }
    if (config.getSystemDebug())
...
{
  if (config.getSystemDebug())
    slog.logln(ntp.iso8601DateTime() + "[ INFO ] EVSE-WiFi is running in Fallback AP Mode");
  WiFi.disconnect(true);
...
void ICACHE_FLASH_ATTR startWebserver()
{
  ...
  server.on(
      ...
      [](AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final)
      {
        ...
        {
          if (config.getSystemDebug())
            slog.log(ntp.iso8601DateTime() + "[ UPDT ] Firmware update started: ");
          if (config.getSystemDebug())
            slog.logln(filename.c_str());
          updateRunning = true;
#ifdef ESP8266
        ...
        if (final)
        {
          ...
          {
            if (config.getSystemDebug())
              slog.log(ntp.iso8601DateTime() + "[ UPDT ] Firmware update finished: ");
            if (config.getSystemDebug())
              slog.logln(String(index + len));
...
void ICACHE_RAM_ATTR setup()
{
  ...
  if (config.getSystemDebug())
    slog.logln("");
  if (config.getSystemDebug())
    slog.log(ntp.iso8601DateTime() + "[ INFO ] EVSE-WiFi - version ");
  if (config.getSystemDebug())
    slog.log(swVersion);
  delay(500);
  ...
  if (config.getEvseLedConfig(0) != 1)
  {
    ...
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] LED pin set");
  }

  // Activate the button pin with pullup in any setup to prevent bouncing pin state
  pinMode(config.getButtonPin(0), INPUT_PULLUP);
  if (config.getSystemDebug())
    slog.log(ntp.iso8601DateTime() + "[ SYSTEM ] Internal pullup for button pin set: ");
  if (config.getSystemDebug())
    slog.logln(config.getButtonPin(0));
  ...
  if (digitalRead(config.getButtonPin(0)) == LOW)
  {
    pinMode(config.getEvseLedPin(0), OUTPUT);
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Button Pressed while boot!");
#ifdef ESP32
    ...
    while (digitalRead(button) == LOW)
    {
      ...
      delay(1000);
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Button is pressed...");
    }
  }
  ...
  if (config.useSMeter)
  {
    ...
    if (config.getSystemDebug())
      slog.log(ntp.iso8601DateTime() + "[ Meter ] Use GPIO/Pin ");
    if (config.getSystemDebug())
      slog.logln(config.getMeterPin(0));
    startTotal = getS0MeterReading();
  }

  pinMode(config.getEvseNumPhasesPin(0), OUTPUT);
  if (config.getSystemDebug())
    slog.log(ntp.iso8601DateTime() + "[ SYSTEM ] Use numPhases GPIO ");
  if (config.getSystemDebug())
    slog.logln(config.getEvseNumPhasesPin(0));
  ...
  if (config.getSystemDebug())
    slog.log(ntp.iso8601DateTime() + "[ SYSTEM ] Use RSE GPIO ");
  if (config.getSystemDebug())
    slog.logln(config.getEvseRsePin(0));
  ...
  startWebserver();
  if (config.getSystemDebug())
    slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] End of setup routine");
  if (config.getEvseRemote(0))
    sliderStatus = false;
...
void ICACHE_RAM_ATTR loop()
{
  ...
  if (inFallbackMode && millis() > 600000)
    toReboot = true;

  if (uptime > 3888000)
  { // auto restart after 45 days uptime
    if (vehicleCharging == false)
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ UPDT ] Auto restarting...");
      delay(1000);
      toReboot = true;
    }
  }
  ...
  {
    if (config.getSystemDebug())
      slog.logln(ntp.iso8601DateTime() + "[ UPDT ] Rebooting...");
    delay(100);
    ESP.restart();
  }
  ...
  if (config.useSMeter && previousMeterMillis < millis() - (meterTimeout * 1000))
  { // Timeout when there is less than ~300 watt power consuption -> 10 sec of no interrupt from meter
  ...
  else
  {
    ...
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ INFO ] WiFi connection successfully reconnected");
    }
  ...
  if (config.getButtonActive(0) && digitalRead(config.getButtonPin(0)) == HIGH && buttonState == LOW)
  {
    ...
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Button released");
      buttonState = HIGH;
#ifdef ESP32
      if (millisOnTimeOled < millis())
      { // oLED is off
  ...
  if (digitalRead(buttonPin) != buttonState)
  {
    ...
    if (digitalRead(buttonPin) != buttonState)
    {
      ...
      // turnOnOled();
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Button pressed...");
    }
  }
  if (digitalRead(buttonPin) == LOW && (millis() - buttonTimer) > 10000 && buttonState == LOW)
  { // Reboot
    delay(70);
    if (digitalRead(buttonPin) == LOW)
    {
      if (config.getSystemDebug())
        slog.logln(ntp.iso8601DateTime() + "[ SYSTEM ] Button Pressed > 10 sec -> Reboot");
      toReboot = true;
    }
  }
  ...
  {
    if (setSmartWb11kWSettings() == false)
      setSmartWb11kWSettings();
  }
  ...
  {
    delay(5);
    oled.showDemo(evseStatus, getChargingTime(), evseAmpsConfig, maxCurrent, numPhasesState, currentKW, meteredKWh, ntp.getUtcTimeNow(), &swVersion, evseActive, timerActive);
    millisUpdateOled = millis() + 3000;
  }
  ...
  if (doCpInterruptCp)
  {
    ...
    if (numPhasesDoSwitch && (numPhasesState != numPhasesDoSwitch) && (evseStatus < 3))
    {
      ...
      {
        if (config.getSystemDebug())
          Serial.printf("[ numPhasesDoSwitch ] need to reactivate EVSE\r\n");
        toActivateEVSE = true;
        needReactivation = false;
      }
      if (config.getSystemDebug())
        Serial.printf("[ numPhasesDoSwitch ] switch phases done\r\n");
    }
